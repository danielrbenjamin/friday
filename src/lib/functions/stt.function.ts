...
export async function fetchSTT(params: STTParams): Promise<string> {
  ...
  try {
    ...
    if (!provider) throw new Error("Provider not provided. Please configure an STT provider in settings.");
    if (!selectedProvider) throw new Error("Selected provider not provided. Please select an STT provider in settings.");
